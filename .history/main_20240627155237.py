...
def main():
    ...
    for t in range(1, T+1):
        ...
        for idx in random_list: # random consumption
            a = agents[idx]
            demand = a.pc * B.deposits[a.id] / F.P # intended demand
            demand_actual = np.min(demand, F.G) # actual demand
            consump_actual = demand_actual * F.P # actual consumption
            B.deposits[a.id] -= consump_actual # consumption
        for a in agents: # consumption
            # dj = cj / P = pcj * sj / P
            # dj^ = min(dj, G)
            # cj^ = dj^ * P
            pass
