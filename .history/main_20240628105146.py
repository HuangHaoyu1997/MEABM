...
class agent:
    ...
    def work_adjustment(self, deposit:float, rate:float):
        self.pw = pow(self.z / (deposit + deposit*rate), self.gamma)
    
    def consume_adjustment(self, P:float, deposit:float):
        self.pc = pow(P / (deposit + self.z), self.beta)
class bank:
...
def main():
    ...
    agents = [agent(id=i, pw=np.random.uniform(0.3, 0.7), pc=np.random.uniform(0.2, 0.4), gamma=0.5, ) for i in range(N)]
    for a in agents:
        a.w = beta_dist()
        B.deposits[a.id] = 0.
