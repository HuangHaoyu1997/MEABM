...
class market:
    ...
    def total_intended_demand(self, agent_list:list[agent], P:float, bank:bank) -> float:
        '''
        社会总预期需求 = 消费比例 * 储蓄量 / 价格
        '''
        cnt = np.sum([a.pc * bank.deposits[a.id] / P for a in agent_list])
        return cnt
    
    def imbalance(self, agent_list:list[agent], P:float, bank:bank) -> float:
        '''
        计算 预期需求与实际产量之间的不均衡
        '''
        D = self.total_intended_demand(agent_list, P, bank)
