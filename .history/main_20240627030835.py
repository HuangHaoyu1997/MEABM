...
class market:
    def __init__(self, ) -> None:
        self.brackets = list(np.array([0, 97, 394.75, 842, 1607.25, 2041, 5103])*100/12) # monthly income brackets
    ...
    def taxation(self, wages:list[float]):
        for w in wages:
            ...
            for i in range(len(self.brackets) - 1):
                if income > self.brackets[i + 1]:
                    tax += (self.brackets[i + 1] - self.brackets[i]) * rates[i]
                else:
                    tax += (income - self.brackets[i]) * rates[i]
                    break
            else:
                tax += (income - self.brackets[-1]) * rates[-1]
...
def main():
    ...
    work_state_history = []
    work_state_history.append([a.l for a in agents]) # t=0 work state
