...
class bank:
    ...
    def rate_adjustment(self, unemployment_rate:float, inflation_rate:float):
        self.rate = np.max(self.rn + self.pi_t + self.alpha_pi * (inflation_rate - self.pi_t) + self.alpha_u * (self.un - unemployment_rate), 0)
...
def main():
    ...
    for t in range(1, T+1):
        ...
        #############################
        # annual operation price and wage adjustment #
        #############################
        if t % 12 == 0:
