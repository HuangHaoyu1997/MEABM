    ...
    for t in range(1, config.num_time_steps+1):
        ...
        if t % 6 == 0 and t > 60:
            for a in agents:
                ...
                sgn = -1 if this_period_deposit >= last_period_deposit else 1
                if sgn == 1: print(t, a.id, sgn)
                a.pw += sgn * np.random.uniform() * 0.05
...
if __name__ == '__main__':
    ...
    def total_deposit(deposits:dict):
        return sum([d[id] for id in deposits.keys()])
    ...
    axs[2, 0].plot([log[key]['deposit'] for key in log.keys()])
    axs[2, 0].set_xlabel('Time / Month'); axs[2, 0].set_ylabel('Production')
    axs[2, 0].grid()
