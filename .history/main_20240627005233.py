...
class market:
    ...
    def total_demand(self, agent_list:list[agent], firm:firm) -> float:
        cnt = 0
        for a in agent_list:
            cnt += (a.pc * self.bank.deposits[a.id] / firm.P)
        return cnt
    
    def imbalance(self, firm:firm):
        D = self.total_demand()
        phi_bar = (D - self.firm.G) / max(D, self.firm.G)
        return phi_bar
