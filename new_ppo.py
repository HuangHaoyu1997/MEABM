...
class Args:
    ...
    num_steps = 500  # 每个episode最大rollout步数（当episode未提前结束时，rollout的数据条数即为num_steps）
    anneal_lr = True
    gae = True
    gae_lambda = 0.9 # 0.95
    gamma = 0.99
...
if __name__ == "__main__":
    ...
    for episode in range(1, args.total_episodes + 1):
        ...
        # 当前episode的实际rollout步数
        rollout_steps = len(rewards)
        # 将列表转换为Tensor，并打包成批次数据
        # 注意：obs中每个元素的shape为env.observation_space.shape
        b_obs = torch.stack(obs)           # shape: (rollout_steps, obs_dim...)
        b_logprobs = torch.stack(logprobs).reshape(-1)  # shape: (rollout_steps,)
        b_actions = torch.stack(actions)   # shape: (rollout_steps, action_dim)
        b_rewards = torch.stack(rewards).reshape(-1)    # shape: (rollout_steps,)
        b_dones = torch.stack(dones).reshape(-1)        # shape: (rollout_steps,)
        b_values = torch.stack(values).reshape(-1)      # shape: (rollout_steps,)
        ...
        # 将数据平铺，以便于后续小批量训练
        # 当前batch_size等于rollout_steps（可能小于args.num_steps）
