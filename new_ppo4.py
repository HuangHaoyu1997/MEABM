...
class Args:
    ...
    num_steps = 200 # the number of steps to run in each environment per policy rollout
    anneal_lr = True
    gae = True
    gae_lambda = 0.9
    gamma = 0.99
...
def make_env(env_id, step_len, seed):
    def thunk():
        env = MEABM_gym(event=env_id, step_len=step_len, seed=seed)
        # env = gym.make(env_id)
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_A = self.layer_init(nn.Linear(64, action_dim), std=0.01)
        self.actor_B = self.layer_init(nn.Linear(64, action_dim), std=0.01)
...
if __name__ == "__main__":
    ...
    envs = gym.vector.AsyncVectorEnv( # AsyncVectorEnv( SyncVectorEnv
        [make_env(args.env_id, 50, args.seed + i) for i in range(args.num_envs)]
    )
    ...
    plt.ion()
    fig, ax = plt.subplots() 
    ax.set_xlabel("Episode"); ax.set_ylabel("Cumulative Reward"); ax.set_title("Training Trend"); ax.grid()
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            next_obs, reward, done, trun, info = envs.step(action.cpu().numpy()*0.7-0.2)
            ...
            if info != {}:
                ...
                for item in info:
                    if item == "episode":
                        ...
                        print(f"global_step={global_step}, episodic_return={rewards_[rewards_!=0]}, time={time.time()-start_time:.4f}")
                        all_rewards.append(np.mean(rewards_))
                        ax.clear()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                print('b_obs[mb_inds]:', torch.isnan(b_obs[mb_inds]).any())
